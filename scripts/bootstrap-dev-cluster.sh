...
# The recovery keys, kept rather than dropped on the floor.
#
# With a seal stanza, `operator init` returns recovery keys instead of
# unseal keys: Vault unseals itself from Transit, and these exist for the
# operations that still need a quorum of humans -- `operator generate-root`
# and `operator rekey`.
#
# This script used to keep only the root token, which made the local
# profile unable to demonstrate either. Worse, it made revoking the root
# token -- the thing a real deployment is supposed to do once its auth
# methods are configured -- a one-way door: no recovery keys, no way to
# generate a new root.
#
# Not on stdout. The contract above is that stdout carries the root token
# and nothing else, so these go to a file and the path is logged to
...
log "  Gitignored, dev-only, and regenerated by every bootstrap."
