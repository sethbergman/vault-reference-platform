...
if [[ "$CLOUD" == "aws" ]]; then
    log "KMS key: scheduled for deletion, not deleted."
    log "  deletion_window_in_days is 7 (terraform/aws/main.tf). The key"
    log "  stays in PendingDeletion until then. It costs nothing in that"
    log "  state, and it can be cancelled if you destroyed by mistake —"
    log "  which is the reason for the window."
else
