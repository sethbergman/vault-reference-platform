terraform {
  # Still 1.7, where terraform/aws now requires 1.10. The azurerm backend
  # locks with a blob lease and has done for years, so nothing here needs
  # the version that introduced S3 native locking. Two backends, two
  # locking mechanisms, and only one of them has a floor worth stating.
  required_version = ">= 1.7"

  # Deliberately empty; the account, container and key are supplied at
  # init time from terraform/azure/backend.hcl, generated by
  # terraform/azure/bootstrap and gitignored:
  #
  #   terraform init -backend-config=backend.hcl
  #
  # The account has to exist before this runs — see
  # docs/terraform-state.md, and the note in terraform/aws/main.tf for
  # why the block is empty rather than filled in.
  backend "azurerm" {}
